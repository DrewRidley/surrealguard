...
The `surrealguard` CLI: a static analyzer and type-inference engine for
...
[GitHub Release](https://github.com/DrewRidley/surrealguard/releases) for **this
...
pins the release tag, `npx surrealguard@0.4.0` always gets the 0.4.0 binary — the
...
npx surrealguard check     # analyze; non-zero exit if errors remain
npx surrealguard generate  # emit the typed TypeScript client
```

`check` also takes `--json` for machine-readable diagnostics, and `generate`
takes `--out <path>`. Run `surrealguard <command> --help` for the flags your
...
is at [github.com/DrewRidley/surrealguard](https://github.com/DrewRidley/surrealguard).

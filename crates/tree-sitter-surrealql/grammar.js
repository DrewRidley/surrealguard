...
export default grammar({
	...
	rules: {
		...
		InsertStatement: ($) =>
			seq(
				...
				choice(
					...
					// ambiguous until the token after the closing paren.
					alias(
						seq('(', $._subqueryStatement, ')'),
						$.SubQuery,
					),
					seq(
						'(',
		...
		// keyword: `FETCH RETURN` fetches a field named `RETURN`.
		FetchClause: ($) =>
			seq(
		...
		_fetchIdiom: ($) =>
			seq(
				choice($.Ident, alias($._any_kw, $.Ident)),
				repeat(
					choice(

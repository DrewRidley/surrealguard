...
const ENTRIES: &[(u16, &str, Severity)] = &[
    ...
    (
        2001,
        "assignability contract: a SET value must inhabit the field's declared type",
        Severity::Error,
    ),
    ...
    (
        2004,
        "operand contract: the operands must make sense together for the operator",
        Severity::Error,
    ),
    ...
    (
        2017,
        "ORDER BY availability contract: each key names a field of the result rows (or RAND())",
        Severity::Error,
    ),

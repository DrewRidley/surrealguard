...
impl Lsp {
    ...
    fn start() -> Self {
        Lsp::handshake(json!({"capabilities": {}}))
    }
    ...
    fn handshake(params: Value) -> Self {
    ...
    /// The next message that is *for us to interpret*.
    ///
    /// A server→client **request** carries an `id` too, so without this a
    /// `client/registerCapability` whose id happened to match an outstanding
    /// request of ours would be mistaken for its response. Those are answered
    /// with `null` — which is what a real editor's reply amounts to here — and
    /// skipped.
    ...
    /// Replaces a document's whole text and returns the diagnostics that
    /// follow — how a round trip checks that an applied edit really worked.
...
fn a_surql_diagnostic_offers_both_suppressions_and_the_inline_one_actually_silences_it() {
    ...
    let after = lsp.did_change(&query_uri, &suppressed);
    assert!(
...
fn a_multi_line_template_takes_an_inline_directive_that_silences_it() {
    ...
    let after = lsp.did_change(&module_uri, &suppressed);
    assert!(
        !after.iter().any(|d| d["code"] == "E1001"),

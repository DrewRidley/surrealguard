...
pub fn analyze_upsert(ctx: &mut AnalysisContext<'_>, stmt: &ast::UpsertStmt) -> Kind {
    upsert_response_kind(
        stmt,
        ctx.source(),
        ctx.source_text(),
        ctx.schema(),
...
pub(crate) fn upsert_response_kind(
    stmt: &ast::UpsertStmt,
    source: &SourceId,
    text: &str,
    schema: &SchemaIndex,
...
mod tests {
    ...
    fn analyze(schema: &SchemaIndex, query: &str) -> Kind {
        ...
        upsert_response_kind(&stmt, parsed.source_id(), parsed.text(), schema, &env)
    }

...
use crate::select_ir::{select_ir_from_statement, SelectModifier};
...
pub struct SourceOrderedSemanticOutput {
    ...
    /// The full constraint set per source-scoped parameter (kind + value
    /// domain + origins) — the host-adapter export.
    pub param_constraints: Vec<(SourceId, ParamInference)>,
...
pub fn analyze_sources_in_source_order(
    ...
    for parsed in parsed_sources {
        ...
        for param in analyzer_env.params() {
            output
                .param_constraints
                .push((parsed.source_id().clone(), param));
        }

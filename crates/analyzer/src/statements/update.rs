    ...
    if let Some(tbl) = table_ref {
        if ctx.strict && !ctx.has_table(tbl) {
            let span = find_all(node, "identifier")
                ...
                .or_else(|| child_by_kind(node, "update_target").map(|t| Span::from_node(&t)))
...
mod tests {
    ...
    #[test]
    fn update_infer_set_param() {
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD age ON user TYPE int;
            UPDATE user SET age = $new_age;
            "#,
        )
        .unwrap();
        ...
        let has_param = params
            .iter()
            .any(|(n, t)| n == "new_age" && *t == Kind::Int);
        assert!(has_param, "Should infer $new_age as int, got {:?}", params);
    }

    #[test]
    fn update_infer_content_param() {
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
        ...
        .unwrap();
        let params = result.context.inferred_params();
        assert!(!params.is_empty(), "Should infer parameter type for $data");
    }

    #[test]
    fn update_plus_eq_array_element_valid() {
        // += on array<string> with a string element should be valid (append)
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD tags ON user TYPE array<string>;
            UPDATE user SET tags += "new";
            "#,
        )
        .unwrap();
        let errors: Vec<_> = result
        ...
        assert!(
            errors.is_empty(),
            "tags += 'new' should be valid (append string to array<string>): {:?}",
            errors
        );
    }

    #[test]
    fn update_plus_eq_numeric_valid() {
        // += on int with an int value should be valid (addition)
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD age ON user TYPE int;
        ...
        assert!(
            errors.is_empty(),
            "age += 1 should be valid (add int to int): {:?}",
            errors
        );
    }

    #[test]
    fn update_plus_eq_numeric_invalid() {
        // += on int with a string value should be invalid
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD age ON user TYPE int;
            UPDATE user SET age += "text";
            "#,
        )
        .unwrap();
        let errors: Vec<_> = result
        ...
        assert!(
            !errors.is_empty(),
            "age += 'text' should be invalid (string to int with +=): {:?}",
            result.diagnostics
        );
    }

    #[test]
    fn update_minus_eq_array_element_valid() {
        // -= on array<string> with a string element should be valid (remove)
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD tags ON user TYPE array<string>;
            UPDATE user SET tags -= "old";
            "#,
        )
        .unwrap();
        let errors: Vec<_> = result
        ...
        assert!(
            errors.is_empty(),
            "tags -= 'old' should be valid (remove string from array<string>): {:?}",
            errors
        );
    }

    #[test]
    fn update_minus_eq_numeric_valid() {
        // -= on int with an int value should be valid (subtraction)
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD score ON user TYPE int;
            UPDATE user SET score -= 5;
            "#,
        )
        .unwrap();
        let errors: Vec<_> = result
        ...
        assert!(
            errors.is_empty(),
            "score -= 5 should be valid (subtract int from int): {:?}",
            errors
        );
    }

    #[test]
    fn update_minus_eq_string_invalid() {
        // -= on string should be invalid (subtraction not supported on strings)
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
        ...
        assert!(
            !errors.is_empty(),
            "name -= 'x' should be invalid (string does not support -=): {:?}",
            result.diagnostics
        );
    }

    #[test]
    fn update_plus_eq_array_concat_valid() {
        // += on array<string> with an array<string> value should be valid (concat)
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD tags ON user TYPE array<string>;
            UPDATE user SET tags += ["a", "b"];
            "#,
        )
        .unwrap();
        let errors: Vec<_> = result
        ...
        assert!(
            errors.is_empty(),
            "tags += ['a','b'] should be valid (concat arrays): {:?}",
            errors
        );
    }

    #[test]
    fn update_plus_eq_string_valid() {
        // += on string with a string value should be valid (concatenation)
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD bio ON user TYPE string;
            UPDATE user SET bio += " more text";
            "#,
        )
        .unwrap();
        let errors: Vec<_> = result
        ...
        assert!(
            errors.is_empty(),
            "bio += ' more text' should be valid (string concatenation): {:?}",
            errors
        );
    }

    #[test]
    fn update_plus_eq_string_with_int_invalid() {
        // += on string with an int value should be invalid
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
            UPDATE user SET name += 42;
            "#,
        )
        .unwrap();
        let errors: Vec<_> = result
        ...
        assert!(
            !errors.is_empty(),
            "name += 42 should be invalid (int to string with +=): {:?}",
            result.diagnostics
        );
    }

    #[test]
    fn update_nested_field_on_string_warns() {
        // Setting name.first where name is string should warn
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
            UPDATE user SET name.first = "John";
            "#,
        )
        .unwrap();
        let warnings: Vec<_> = result
            .diagnostics
            .iter()
            .filter(|d| d.code == crate::Code::InvalidNestedAccess)
            .collect();
        assert!(
            !warnings.is_empty(),
            "Expected warning for nested access on string field, got: {:?}",
            result.diagnostics
        );
        assert!(
            warnings[0].message.contains("name.first"),
            "Warning should mention the full path 'name.first', got: {}",
            warnings[0].message
        );
    }

    #[test]
    fn update_array_index_no_error() {
        // Setting tags[0] where tags is array<string> should not error
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD tags ON user TYPE array<string>;
            UPDATE user SET tags[0] = "new";
            "#,
        )
        .unwrap();
        let errors: Vec<_> = result
            .diagnostics
            .iter()
            .filter(|d| d.is_error() || d.code == crate::Code::InvalidNestedAccess)
            .collect();
        assert!(
            errors.is_empty(),
            "Array indexing on array<string> should be valid: {:?}",
            errors
        );
    }

    #[test]
    fn update_nested_field_on_object_no_warn() {
        // Setting address.city where address is object should not warn
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD address ON user TYPE object;
            UPDATE user SET address.city = "NYC";
            "#,
        )
        .unwrap();
        let warnings: Vec<_> = result
        ...
        assert!(
            warnings.is_empty(),
            "Nested access on object field should be valid: {:?}",
            warnings
        );
    }

    #[test]
    fn update_nested_field_on_int_warns() {
        // Setting age.value where age is int should warn
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD age ON user TYPE int;
            UPDATE user SET age.value = 5;
            "#,
        )
        .unwrap();
        let warnings: Vec<_> = result
        ...
        assert!(
            !warnings.is_empty(),
            "Expected warning for nested access on int field, got: {:?}",
            result.diagnostics
        );
    }

    #[test]
    fn update_nested_field_on_schemaless_no_warn() {
        // Schemaless tables should allow nested access without warnings
        let result = analyze(
            r#"
        ...
        assert!(
            warnings.is_empty(),
            "Schemaless table should not warn on nested access: {:?}",
            warnings
        );
    }

    #[test]
    fn patch_valid_operations_no_error() {
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
            UPDATE user PATCH [{"op": "replace", "path": "/name", "value": "new"}];
            "#,
        )
        .unwrap();
        ...
        assert!(
            ...
            "Valid PATCH should not produce errors: {:?}",
            patch_errors
        );
    }

    #[test]
    fn patch_invalid_op_warns() {
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
            UPDATE user PATCH [{"op": "invalid_op", "path": "/name"}];
            "#,
        )
        .unwrap();
        let patch_errors: Vec<_> = result
        ...
        assert!(
            !patch_errors.is_empty(),
            "Invalid PATCH op should produce a warning, got: {:?}",
            result.diagnostics
        );
        assert!(
            patch_errors[0].message.contains("invalid_op"),
            "Warning should mention the invalid op: {}",
            patch_errors[0].message
        );
    }

    #[test]
    fn patch_missing_op_warns() {
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
            UPDATE user PATCH [{"path": "/name", "value": "new"}];
            "#,
        )
        .unwrap();
        let patch_errors: Vec<_> = result
        ...
        assert!(
            !patch_errors.is_empty(),
            "PATCH object missing op should produce a warning, got: {:?}",
            result.diagnostics
        );
        assert!(
            patch_errors[0].message.contains("missing"),
            "Warning should mention missing op: {}",
            patch_errors[0].message
        );
    }

    #[test]
    fn patch_multiple_valid_ops() {
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
        ...
        assert!(
            patch_errors.is_empty(),
            "All valid PATCH ops should not produce errors: {:?}",
            patch_errors
        );
    }

    #[test]
    fn patch_with_variable_no_error() {
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
            UPDATE user PATCH $patches;
            "#,
        )
        .unwrap();
        let patch_errors: Vec<_> = result
        ...
        assert!(
            patch_errors.is_empty(),
            "PATCH with variable should not produce patch validation errors: {:?}",
            patch_errors
        );
    }

    #[test]
    fn update_duplicate_field_assignment_warns() {
        let result = analyze(
            r#"
            DEFINE TABLE user SCHEMAFULL;
            DEFINE FIELD name ON user TYPE string;
            UPDATE user SET name = 'John', name = 'Jane';
            "#,
        )
        .unwrap();
        let warnings: Vec<_> = result
        ...
        assert!(
            !warnings.is_empty(),
            "Expected duplicate field assignment warning for UPDATE, got: {:?}",
            result.diagnostics
        );
    }
}

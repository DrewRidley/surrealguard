...
| H-2 | ~~**`COMPUTED <~edge` is `unknown` in the schema index when the edge is declared later**~~ — **FIXED.** An untyped field's stored-value kind is a global property, not an ordering contract, so `infer_field_value_kind` now resolves against the whole-workspace catalog (H-1's mechanism) instead of the incremental one. Fixes all three faces: the order-dependent kind, the schema-index/query-path disagreement, and a false E1001 on a same-file `<~t` forward reference. | medium |

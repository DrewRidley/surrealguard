# crates.io release runbook — SurrealQL Analyzer 0.3.0

Ordered runbook for publishing the whole SurrealQL Analyzer tree to crates.io at
`0.3.0`, matching the already-published `@surrealdb/analyzer-*` npm 0.3.0.
...
- `[workspace.package] version = "0.3.0"` in the root `Cargo.toml`; every
  ...
  `surrealql-analyzer-wasm` crate stays `version = "0.1.0"` / `publish = false` and is
  **not** published.
- Logged in: `cargo login <token>` (a token with publish scope for the
  `DrewRidley`-owned crates). `surrealql-analyzer`, `surrealql-analyzer-macros`, etc. already
...
- Verified locally (all green):
  ...
  - `CARGO_TARGET_DIR=/tmp/sg-release cargo test --doc -p surrealql-analyzer-rs -p surrealql-analyzer-macros -p surrealql-analyzer`
  - `cargo package -p surrealql-analyzer-tree-sitter-surrealql` and
    `cargo package -p surrealql-analyzer-embed` package **and verify** cleanly.
...
```bash
# 1. grammar — leaf, no SurrealQL Analyzer deps. MUST ship the vendored C sources.
cargo publish -p surrealql-analyzer-tree-sitter-surrealql

# 2. syntax — depends on the grammar (1).
cargo publish -p surrealql-analyzer-syntax

# 3. diagnostics — depends on syntax (2).
cargo publish -p surrealql-analyzer-diagnostics

# 4. workspace — depends on diagnostics (3) + syntax (2).
cargo publish -p surrealql-analyzer-workspace

# 5a. codegen — depends on workspace (4).
cargo publish -p surrealql-analyzer-codegen

# 5b. embed — leaf, no SurrealQL Analyzer deps; publishable any time after login.
cargo publish -p surrealql-analyzer-embed

# 6. macros — depends on workspace (4) + diagnostics (3).
cargo publish -p surrealql-analyzer-macros

# 7. rs — depends on macros (6). The flagship SDK crate.
cargo publish -p surrealql-analyzer-rs

# 8. cli (package name `surrealql-analyzer`) — depends on codegen (5a), diagnostics (3),
#    embed (5b), syntax (2), workspace (4).
cargo publish -p surrealql-analyzer

# 9. lsp — depends on diagnostics (3), embed (5b), syntax (2), workspace (4).
cargo publish -p surrealql-analyzer-lsp
```

That is the full set — 10 crates. `surrealql-analyzer-wasm` is intentionally omitted
(`publish = false`).
...
- **surrealql-analyzer-tree-sitter-surrealql** — the vendored grammar. The published
...
- **surrealql-analyzer-embed** — no SurrealQL Analyzer deps (only `tree-sitter` +
  `tree-sitter-typescript`); packages and verifies standalone.
- **surrealql-analyzer-rs** — the front-page SDK. Re-exports the `query!`/`surql!`
...
- **surrealql-analyzer (cli)** — binary crate; carries `[package.metadata.binstall]`
  so `cargo binstall surrealql-analyzer` pulls the prebuilt GitHub Release asset. The
  asset naming there MUST stay in lockstep with the npm launcher and
  `.github/workflows/release.yml` (`v{version}` tag,
  `surrealql-analyzer-{version}-{target}.{tgz,zip}`).
- **surrealql-analyzer-workspace / -syntax / -diagnostics / -codegen / -lsp** — public
...
- docs.rs builds each crate on publish and renders the `//!` crate docs as the
  front page. `surrealql-analyzer-rs` carries the richest page (macros, schema
...
- After publishing, spot-check <https://docs.rs/surrealql-analyzer-rs> and
  <https://docs.rs/surrealql-analyzer> rendered correctly.

## Post-publish

- Tag the release (`git tag v0.3.0 && git push --tags`) so the binstall asset
  URLs resolve and the GitHub Release workflow runs.
- Confirm `cargo install surrealql-analyzer` and `cargo add surrealql-analyzer-rs` both
